...
export default function HrLoginPage() {
  ...
  const [next, setNext] = useState("/hr");
    ...
    <section className="hero-gradient flex min-h-[70vh] items-center justify-center px-4 py-16">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-white p-8 card-shadow">
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-3 text-xl font-bold text-ink">HR sign in</h1>
          <p className="mt-1 text-sm text-muted">Enter your HR access code to open the onboarding tools.</p>
        </div>
        <form onSubmit={onSubmit} className="space-y-4">
          <label className="block">
            <span className="mb-1 block text-sm font-semibold text-ink-soft">HR access code</span>
            <input
              type="password"
              value={code}
